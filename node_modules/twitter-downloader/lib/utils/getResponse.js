"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TwitterDL = void 0;
const axios_1 = __importDefault(require("axios"));
const https_proxy_agent_1 = require("https-proxy-agent");
const socks_proxy_agent_1 = require("socks-proxy-agent");
const getGuestToken_1 = require("./getGuestToken");
const getAuthorization_1 = require("./getAuthorization");
const params_1 = require("../contants/params");
const api_1 = require("../contants/api");
const millsToMinutesAndSeconds = (millis) => {
    const minutes = Math.floor(millis / 60000);
    const seconds = ((millis % 60000) / 1000).toFixed(0);
    return `${minutes}:${Number(seconds) < 10 ? "0" : ""}${seconds}`;
};
const TwitterDL = (url, config, proxy) => new Promise(async (resolve) => {
    const id = url.match(/\/([\d]+)/);
    const regex = /^(https?:\/\/)?(www\.)?(m\.)?twitter|x\.com\/\w+/;
    if (!regex.test(url))
        return resolve({ status: "error", message: "Invalid URL!" });
    if (!id)
        return resolve({
            status: "error",
            message: "There was an error getting twitter id. Make sure your twitter url is correct!",
        });
    const guest_token = await (0, getGuestToken_1.getGuestToken)();
    const csrf_token = config?.cookie
        ? config.cookie.match(/(?:^|; )ct0=([^;]*)/)
        : "";
    if (!guest_token)
        return resolve({
            status: "error",
            message: "Failed to get Guest Token. Authorization is invalid!",
        });
    (0, axios_1.default)((0, api_1._twitterapi)(api_1._twitterPostID, api_1._tweetresultbyrestid), {
        method: "GET",
        params: {
            variables: JSON.stringify((0, params_1.variables)(id[1])),
            features: JSON.stringify(params_1.features),
        },
        headers: {
            Authorization: config?.authorization
                ? config.authorization
                : await (0, getAuthorization_1.getTwitterAuthorization)(),
            Cookie: config?.cookie ? config.cookie : "",
            "x-csrf-token": csrf_token ? csrf_token[1] : "",
            "x-guest-token": guest_token,
            "user-agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.132 Safari/537.36",
        },
        httpsAgent: (proxy &&
            (proxy.startsWith("socks")
                ? new socks_proxy_agent_1.SocksProxyAgent(proxy)
                : proxy.startsWith("http")
                    ? new https_proxy_agent_1.HttpsProxyAgent(proxy)
                    : undefined)) ||
            undefined,
    })
        .then(({ data }) => {
        if (!data.data.tweetResult?.result) {
            return resolve({
                status: "error",
                message: "Tweet not found!",
            });
        }
        if (data.data.tweetResult.result?.reason === "NsfwLoggedOut") {
            return resolve({
                status: "error",
                message: "This tweet contains sensitive content! Please use cookies to avoid errors!",
            });
        }
        const result = data.data.tweetResult.result.__typename ===
            "TweetWithVisibilityResults"
            ? data.data.tweetResult.result.tweet
            : data.data.tweetResult.result;
        const statistics = {
            reply_count: result.legacy?.reply_count || 0,
            retweet_count: result.legacy?.retweet_count || 0,
            favorite_count: result.legacy?.favorite_count || 0,
            bookmarkCount: result.legacy?.bookmark_count || 0,
            viewCount: Number(result.views?.count || 0),
        };
        const user = result.core?.user_results?.result;
        const author = {
            username: user?.legacy?.screen_name || '',
            bio: user?.legacy?.description || '',
            possiblySensitive: user?.legacy?.possibly_sensitive || false,
            verified: user?.legacy?.verified || false,
            location: user?.legacy?.location || '',
            profileBannerUrl: user?.legacy?.profile_banner_url || '',
            profileImageUrl: user?.legacy?.profile_image_url_https || '',
            url: "https://twitter.com/" + (user?.legacy?.screen_name || ''),
            statistics: {
                favoriteCount: user?.legacy?.favourites_count || 0,
                followersCount: user?.legacy?.followers_count || 0,
                friendsCount: user?.legacy?.friends_count || 0,
                statusesCount: user?.legacy?.statuses_count || 0,
                listedCount: user?.legacy?.listed_count || 0,
                mediaCount: user?.legacy?.media_count || 0,
            },
        };
        const getNormalizedMedia = (mediaArray) => {
            return mediaArray?.map((v) => {
                if (v.type === "photo") {
                    return {
                        type: v.type,
                        image: v.media_url_https,
                        expandedUrl: v.expanded_url,
                    };
                }
                else if (v.video_info) {
                    const isGif = v.type === "animated_gif";
                    const videos = v.video_info?.variants
                        .filter((video) => video.content_type === "video/mp4")
                        .map((variants) => {
                        let quality = isGif
                            ? `${v.original_info?.width || ''}x${v.original_info?.height || ''}`
                            : variants.url.match(/\/([\d]+x[\d]+)\//)?.[1];
                        return {
                            bitrate: variants.bitrate,
                            content_type: variants.content_type,
                            quality,
                            url: variants.url,
                        };
                    });
                    return {
                        type: isGif ? "animated_gif" : "video",
                        cover: v.media_url_https,
                        duration: millsToMinutesAndSeconds(v.video_info?.duration_millis || 0),
                        expandedUrl: v.expanded_url,
                        videos,
                    };
                }
                return null; // Return null for unrecognized media types
            }).filter(Boolean) || [];
        };

        let media = getNormalizedMedia(result.legacy?.entities?.media);

        if (media.length === 0 && result.extended_entities?.media) {
            media = getNormalizedMedia(result.extended_entities.media);
        }
        resolve({
            status: "success",
            result: {
                id: result.legacy?.id_str || '',
                createdAt: result.legacy?.created_at || '',
                description: result.legacy?.full_text || '',
                languange: result.legacy?.lang || '',
                possiblySensitive: result.legacy?.possibly_sensitive || false,
                possiblySensitiveEditable: result.legacy?.possibly_sensitive_editable || false,
                isQuoteStatus: result.legacy?.is_quote_status || false,
                mediaCount: media.length,
                author,
                statistics,
                media,
            },
        });
    })
        .catch((e) => {
        return resolve({
            status: "error",
            message: e.message,
        });
    });
});
exports.TwitterDL = TwitterDL;
