...
app.get('/api/fetch', async (req, res) => {
  ...
  if (!twitterRegex.test(tweetUrl)) {
    return res.status(400).json({ success: false, error: 'Invalid X/Twitter Tweet URL. Make sure it is a valid status URL.' });
  }
  ...
  try {
    ...
    if (result && result.status === 'success' && result.result) {
      ...
      if (!videoMedia) {
        return res.status(404).json({ success: false, error: 'No video or GIF found in this tweet.' });
      }
      ...
      const response = {
        ...
        thumbnail: data.thumbnail || (videoMedia.expanded_url ? videoMedia.expanded_url : ''),
        videos: []
      };
      ...
      return res.status(400).json({ 
        success: false, 
        error: (result && result.message) || 'Failed to fetch video details from X/Twitter.' 
      });
    }
  } catch (error) {
    ...
    return res.status(500).json({ 
      success: false, 
      error: 'Failed to retrieve video. This might be due to X/Twitter rate limits, age-restricted content, or server constraints. Please try again later.' 
    });
  }
});
...
app.get('/api/download', async (req, res) => {
  ...
  try {
    ...
    res.status(500).send('Failed to download video file. The direct link might be expired or restricted.');
  }
});
